...
axios.defaults.baseURL = "https://api.airtable.com/v0/apphPJiqT9qp0ofZO/Table%201";
axios.interceptors.request.use(async (config) => {
  if (!config.headers["Authorization"]) {
    config.headers["Authorization"] = `Bearer keytwg7ToP1vXPH3k`;
  }
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    if (req.method === "POST") {
      ...
      const list = await axios.get("/?maxRecords=10&view=Grid%20view");
